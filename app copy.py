...
def album_card_line(result):
    ...
    return dbc.ListGroupItem(
        ...
        action=True,
        id={"type": "search-result", "album_id": a['album_id']},
...
app.layout = html.Div([
    ...
    # Add a store to keep track of whether we're in search mode
    dcc.Store(id="search-active", data=False),
...
@app.callback(
    ...
    Output("search-active", "data", allow_duplicate=True),
    Input("url","pathname"),
    State("search-active", "data"),
    prevent_initial_call=True
)
def router(pathname:str, search_active):
    # Reset search state when navigating to a specific page
    if not pathname or pathname == "/":
        return [layout_home()], False
    if pathname.startswith("/album/"):
        aid = pathname.split("/album/")[1]
        if aid in ALBUMS: 
            return [layout_album(aid)], False
        else:
            # If album ID is invalid, go back to home
            return [layout_home()], False
...
@app.callback(
    ...
    Output("search-active", "data"),
    Input("search-input","value"),
    State("url","pathname"),
    prevent_initial_call=True
)
def do_search(q, path):
    # Trigger context to check if this is a search input event
    ...
    return [layout_search(results)], True
...
@app.callback(
    Output("url", "pathname"),
    Input({"type": "search-result", "album_id": ALL}, "n_clicks"),
    prevent_initial_call=True
)
def handle_search_result_click(n_clicks_list):
    ...
    try:
        ...
        album_id = json.loads(trigger_id)["album_id"]
